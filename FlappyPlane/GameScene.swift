...
//  FlappyPlane
//
//  Created by Mackley Magalhães da Silva on 04/02/23.
//
...
class GameScene: SKScene, SKPhysicsContactDelegate {
    ...
    var speedRocks: Double = 1
    
    override func didMove(to view: SKView) {
        
        rockPhysics = SKPhysicsBody(texture: rockTexture, size: rockTexture.size())
        groundPhysics = SKPhysicsBody(texture: groundTexture, size: groundTexture.size())
    ...
    func createPlayer() {
        ...
        player.zPosition = 10
        player.position = CGPoint(x: frame.width / 5,
                                  y: frame.height * 0.75)
        ...
        let animation = SKAction.animate(with: [playerTexture, frame3, frame2], timePerFrame: 0.01)
    ...
    func createGround() {
        ...
        for i in 0 ... 1 {
            ...
            ground.physicsBody = groundPhysics.copy() as? SKPhysicsBody
    ...
    func createRocks() {
        ...
        let moveAction = SKAction.moveBy(x: -endPosition,
                                         y: 0,
                                         duration: 6.2)
